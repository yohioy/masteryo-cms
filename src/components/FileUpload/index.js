...
function FileUpload(props) {
  ...
  const handleSubmission = async () => {
    const selectedFilesCopy = Object.assign({}, selectedFiles);
    ...
    if (filesToUpload.length > 0) {
      ...
      for (let i = 0; i < filesToUpload.length; i++) {
        ...
        const selectedItem = filesToUpload[i];

        const selectedItemCopy = Object.assign({}, filesToUpload[i]);
            ...
            >
              ...
              {isSelected ? (
                selectedFiles.filesToUpload.map((item, index) => {
                  const { uploadStatus, file } = item;
                  return (
                    <div key={index}>
                      <p>
                        Filename: {file.name} - {uploadStatus}{' '}
                      </p>
                    </div>
                  );
                })
